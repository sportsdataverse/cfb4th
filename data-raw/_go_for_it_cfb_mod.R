...
model_vars <- pbp %>%
  ...
  dplyr::mutate(first_down_penalty = firstD_by_penalty) %>%
  dplyr::mutate(#yards_gained =
         ...
         yards_gained = dplyr::if_else(yards_gained < -10, -10, yards_gained),
         yards_gained = dplyr::if_else(yards_gained > 65, 65, yards_gained),
         home_total = (spread + over_under) / 2,
         away_total = (over_under - spread) / 2,
         posteam_total = dplyr::if_else(offense_play == home_team, home_total, away_total),
  ...
  dplyr::filter(rush+pass == 1 | first_down_penalty == 1,
         distance > 0,
         yards_to_goal > 0,
         distance <= yards_to_goal,
         !is.na(posteam_spread),
         !is.na(posteam_total)) %>%
  dplyr::mutate(label = as.double(yards_gained)) %>%
  dplyr::select(
...
#   dials::finalize(dials::mtry(), model_vars),
#   # min_n(),
#   # tree_depth(),
#   # learn_rate(),
#   # loss_reduction(),
...
results %>%
  ...
  ggplot2::ggplot(aes(value, logloss, color = parameter)) +
  ggplot2::geom_point(alpha = 0.8, show.legend = FALSE, size = 3) +
  ggplot2::facet_wrap(~parameter, scales = "free_x") +
  ggplot2::labs(x = NULL, y = "logloss") +
...
# ADD EVERYTHING
usethis::use_data(fd_model, punt_df, fg_model, wp_model, ep_model, team_info, best_model, internal = TRUE, overwrite = TRUE)
